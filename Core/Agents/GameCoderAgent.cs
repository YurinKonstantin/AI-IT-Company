...
public sealed class GameCoderAgent : AgentBase
{
    ...
    public GameCoderAgent(IAiProviderFactory factory, AgentConfigStore configStore,
    ...
    protected override string BuildUserPrompt(AgentContext ctx)
    {
        ...
        var existing = ctx.Files.Count > 0
            ? string.Join("\n", ctx.Files.Where(f => f.EndsWith(".cs") || f.EndsWith(".csproj")).Take(30))
            : "(новый проект)";
        ...
        return $"""
            ...
            Сгенерируй полный, собираемый проект MonoGame.
            Не используй .xnb-контент — только процедурные текстуры.
            """;
    }
